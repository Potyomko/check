...
function getStudents() {
    fetch(`${BASE_URL}/students`).then((response)=>response.json()).then(console.log);
}
getStudents();
//   GET /students/:id - повернути студента за ідентифікатором.
function getStudentsById(studentsId) {
    fetch(`${BASE_URL}/students/${studentsId}`).then((response)=>response.json()).then(console.log);
}
...
function addNewStudent(newStudent) {
    ...
    return fetch(`${BASE_URL}/students`, options).then((res)=>res.json()).then(console.log);
}
addNewStudent({
...
function updatedNewStudent(updatedStudent, studentsPutId) {
    ...
    return fetch(url, optionsS).then((res)=>res.json()).then(console.log);
}
